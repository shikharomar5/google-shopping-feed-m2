...
namespace Ced\GShop\Ui\Component\Listing\Columns\Feed;
...
{
    const URL_PATH_SYNC = 'gxpress/product/index';
    ...
    public function getFileLink($filePath)
    {
        ...
        if (isset($filePath) and !empty($filePath)) {
            ...
            $file = $this->dl->getPath('media') . "/gxpress/response/" . $fileName;
            if ($this->file->fileExists($file)) {
                $url = $this->urlBuilder->getBaseUrl() . "pub/media/gxpress/response/" . $fileName;
            } else {
                if ($this->file->fileExists($filePath)) {
                    $cpDir = $this->dl->getPath('media') . "/gxpress/response/";
