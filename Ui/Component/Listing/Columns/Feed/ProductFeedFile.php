...
/**
 * Class FeedFile
 * @package Ced\GShop\Ui\Component\Listing\Columns\JobScheduler
 */
class ProductFeedFile extends Column
{
    ...
    const URL_PATH_RESEND = 'gxpress/product/index';
        ...
        if (isset($filePath) and !empty($filePath)) {
            ...
            if ($this->file->fileExists($file)) {
                ...
                if ($this->file->fileExists($filePath)) {
                    $cpDir = $this->dl->getPath('media') . "/gxpress/";
                    ...
                    if ($this->file->fileExists($cpDir . $fileName)) {
                        $url = $this->urlBuilder->getBaseUrl() . "pub/media/gxpress/" . $fileName;
                    }
                }
            }
        }
