...
namespace Ced\GShop\Controller\Adminhtml\Main;
...
/**
 * Class License
 * @package Ced\GShop\Controller\Adminhtml\Main
 */
class License extends \Magento\Backend\App\Action
{
            ...
            if ($response && isset($response['hash']) && isset($response['level'])) {
                ...
                try {
                    ...
                    if (is_array($valid) &&
                        ...
                        $path = \Ced\GShop\Block\Extensions::HASH_PATH_PREFIX . strtolower($postData['module_name']) . '_hash';
                        $this->_feedHelper->setDefaultStoreConfig($path, $response['hash'], 0);
                        $path = \Ced\GShop\Block\Extensions::HASH_PATH_PREFIX . strtolower($postData['module_name']) . '_level';
    ...
    private function getLicenseActivateUrl()
    {
        if (is_null($this->_licenseActivateUrl)) {
            $this->_licenseActivateUrl = (/*$this->_feedHelper->getStoreConfig(\Ced\GShop\Block\Extensions::LICENSE_USE_HTTPS_PATH) ? */
                'https://'/* : 'http://'*/)
