    ...
    public function execute()
    {
        ...
        if (!empty($ids)) {
            ...
            if (isset($collection) and $collection->getSize() > 0) {
                foreach ($ids as $id) {
                    $this->accModel->load($id)->setAccountStatus(0)->save();
                }
                $this->messageManager->addSuccessMessage(__($collection->getSize(). ' Account(s) Disabled Successfully'));
            } else {
                $this->messageManager->addErrorMessage(__('No product available for Disable.'));
            }
        } else {
            $this->messageManager->addErrorMessage(__('No product available for Disable.'));
