        ...
        return Column.extend(
            {
                ...
                getLabel: function (row) {
                    ...
                    try {
                        if (row.hasOwnProperty(this.index + '_html')) {
                            ...
                            var validation = row[this.index + '_accountvalidation'];
                            if (validation !== "" && validation !== undefined) {
                                var data = $.parseJSON(validation);
                                if (typeof data === 'object') {
                                    var accounts = Object.keys(data).length;
                                    var counter = 0;
                                    var invalid = 0;
                                    $.each(
                                    ...
                                    if (counter === accounts) {
                                        ...
                                        if (this.getShowHeading(row)) {
                                            html += "<td class='cedcommerce errors'>Validation:</td>";
                                        }
                            ...
                            if (row.hasOwnProperty(this.index + '_account_feed_label')) {
                                html += row[this.index + '_account_feed_label'];
                            }
                ...
                getAccountValidation: function (row) {
                    var validation = false;
                    if (row.hasOwnProperty(this.index + '_accountvalidation')) {
                        validation = row[this.index + '_accountvalidation'];
                    }
                    return validation;
                },

                getAccountFeed: function (row) {
                    var feed = false;
                    if (row.hasOwnProperty(this.index + '_account_feed_errors')) {
                        feed = row[this.index + '_account_feed_errors'];
                    }
                    return feed;
                },
                ...
                startView: function (row) {
                    var self = this;
                    if (this.getAccountValidation(row)) {
                        var previewPopup = $('</div>',{id : 'ced_popup'+this.getAccountId(row) });
                        var data = $.parseJSON(this.getAccountValidation(row));
                        ...
                        if (this.getAccountFeed(row)) {
                            var feed = $.parseJSON(this.getAccountFeed(row));console.log(feed);
