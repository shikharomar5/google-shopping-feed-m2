...
class Profileproducts extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function _construct()
    {
        ...
        $this->_init('gxpress_profile_products', 'id');
    ...
    public function deleteFromProfile($productId)
    {
        ...
        $condition = "{$this->getTable('gxpress_profile_products')}.product_id = " . $dbh->quote($productId);
        //. " AND {$this->getTable('GXpress/profileproducts')}.profile_id = " . $dbh->quote($profileId);
        $dbh->delete($this->getTable('gxpress_profile_products'), $condition);
        return $this;
    ...
    {
        if ( $productId > 0 ) {
            $profileTable = $this->getTable('gxpress_profile_products');

            $productProfile = \Magento\Framework\App\ObjectManager::getInstance()->get('Ced\GShop\Model\Profileproducts')->loadByField('profile_id',$profileId);
