...
class Feed extends \Magento\AdminNotification\Model\Feed
{
    ...
    const XML_FEED_URL_PATH     = 'system/gxpress/feed_url';
    const XML_FREQUENCY_PATH    = 'system/gxpress/frequency';
    const XML_LAST_UPDATE_PATH  = 'system/gxpress/last_update';
    ...
    public function checkUpdate()
    {
        ...
        $feedXml = $this->getFeedData($this->_objectManager->get('Ced\GShop\Helper\Feed')->getEnvironmentInformation());

        $allowedFeedType = explode(',', $this->_backendConfig->getValue(self::XML_FEED_TYPES));

        if ($feedXml && $feedXml->channel && $feedXml->channel->item) {
            $installedModules=$this->_objectManager->get('Ced\GShop\Helper\Feed')->getCedCommerceExtensions();
            foreach ($feedXml->channel->item as $item) {

                if(!isset($installedModules[(string)$item->module])) {
                    continue;
                }
                if($this->_objectManager->get('Ced\GShop\Helper\Feed')->isAllowedFeedType($item)) {
            ...
            if($feedData) {
                $this->_cacheManager->save($this->serialize($feedData), 'all_extensions_by_cedcommerce');
            }
    ...
    public function getFeedData($urlParams = array())
    {
        ...
        try {
            $curl->write(Request::METHOD_POST, $this->getFeedUrl(), '1.1', array(), $body);
            $data = $curl->read();
